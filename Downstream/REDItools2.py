...
'''
# Part 1 - Keep RES ; Coverage >= 2 & Freq >= 0.1
for align in aligners:
    ...
    for file in os.listdir(newpath):
        if file == output_dir:
            continue
        newfile = file.replace('_filt_sortrmdup.output', '_processed.output')
        with open (os.path.join(newpath, output_dir, newfile), 'w') as out:
            with open (os.path.join (newpath, file), 'r') as Read:
                for lines in Read:
                    line = lines.strip().split('\t')
                    if line[7] in RES and float (line[4]) >= Coverage and float(line[8]) >= Freq:
                        if align == 'hisat2':
                                line[0] = 'chr' + line[0]
                        out.write ('\t'.join(line) + '\n')
    ...
    for condition in conditions:
        for clone in samples:
            for cutoff in thresholds:
                ...
                res, db, snp, res_list = stats(os.path.join  (newpath, file), support= float(cutoff), tool = 'REDItools2', rediportal=rediportal38)
                # Save data
...
# Save  data in JSON
file_to_save = 'Data_REDItool2.json'
