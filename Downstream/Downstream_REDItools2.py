...
try:
    print ('\n\n############################################')
    depth = int(input('Depth (Enter 0 to exit): '))
    freq = float(input('MAF (Default 0.1): '))
    print ('############################################\n\n')
except:
    print ('Error, setting depth = 2 & freq =0.1')
    depth=2
    freq=0.1
...
Adar_out = [file for file in output if "ADAR" in file and "output" in file]
WT_out = [file for file in output if "WT" in file and "output" in file]
...
while depth != 0:
    ...
    for sample in WT_out:
        data = load_file (os.path.join(sample))
        data_filt = data[data["Coverage-q30"]>=depth]
        data_filt = data_filt[data_filt["Frequency"]>=freq]
    ...
    ## Tmp analysis; AllSubs can have several variants (format: TC AG ...). 
    ## Frequency column only refers to the first substitution, therefore we remove 
    ## the rest from the IDs.
    wt_clean_tmp2 = dict()
    for key in wt_clean_tmp:
        variant = key.split('|')[2]
        if len (variant) > 2:
            new_key = key.split('|')[0] + '|' + key.split('|')[1] + '|' +key.split('|')[2][0:2]
        else:
            new_key = key
        wt_clean_tmp2[new_key] = wt_clean_tmp[key]
    adar_clean_tmp2 = dict()
    for key in adar_clean_tmp:
        variant = key.split('|')[2]
        if len (variant) > 2:
            new_key = key.split('|')[0] + '|' + key.split('|')[1] + '|' +key.split('|')[2][0:2]
        else:
            new_key = key
        adar_clean_tmp2[new_key] = adar_clean_tmp[key]

    # Remove variants from WT present in ADAR
    wt_clean = [res for res in wt_clean_tmp2 if res not in adar_clean_tmp2]
    adar_clean = [res for res in adar_clean_tmp2 if res not in wt_clean_tmp2]
    ...
    # Print output
    print ('\nStarting analysis on REDItools2 output taking 6 samples (x3 WT_mock) & (x3 ADAR1KO_mock)\n')
    print ('\nStep 1: Load data + Filtering (depth >= {} & freq >= {})\n'.format(depth, freq))
